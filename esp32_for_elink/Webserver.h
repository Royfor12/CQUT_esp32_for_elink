...
const char* apSSID = "CQUT日历配网WiFi";
boolean settingMode;
String ssidList;
String xz_code;
DNSServer dnsServer;
...
void startWebServer() {  
  if (settingMode) {
    ...
    webServer.on("/settings", []() {
      String s = "<head><meta charset=\"utf-8\"></head><h1>CQUT日历Wi-Fi配置</h1><p>请在选择WiFi名称后输入对应的WiFi密码.</p>";
      ...
      webServer.send(200, "text/html", makePage("CQUT日历Wi-Fi配置", s));
    });
    webServer.on("/setap", []() {
...
void setupMode() {
  ...
  WiFi.softAP(apSSID);
  dnsServer.start(53, "*", apIP);//将所有地址映射到esp32的ip
  /*如果联网失败就显示失败提醒*/
